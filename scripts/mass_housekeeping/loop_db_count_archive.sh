...
tb_list=`db2 -x "select distinct rtrim(tabschema) || '.' || rtrim(tabname) from syscat.tables where tabschema in (select distinct tabschema from syscat.tables where tabschema in  ('ARCHIVE')) and type = 'T' " ` ;
