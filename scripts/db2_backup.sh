...
typeset    cBackupDir="/shared/db2/backups"
  ...
  function determineRetentionPeriod {
    ...
    case ${lHostName} in
      ...
      sla70190 )
          # ABS Development
          lBackupRetentionPeriod=${cBackupRetention}
          [[ "${lDatabase}" == "DGBNA195" ]] && lBackupRetentionPeriod=15
          lBackupRetentionPeriodDb2=$(( lBackupRetentionPeriod + 2 ))
        ;;
      sla71147 )
          # ABS Dev + Test on new server
          lBackupRetentionPeriod=${cBackupRetention}
	  [[ "${lDatabase}" == "DGBNA220" ]] && lBackupRetentionPeriod=15
          lBackupRetentionPeriodDb2=$(( lBackupRetentionPeriod + 2 ))
        ;;
      sla70191 )
          # ABS Test + NFT
      ...
      sla70192 )
          # ABS UAT
          lBackupRetentionPeriod=${cBackupRetention}
          lBackupRetentionPeriodDb2=$(( lBackupRetentionPeriod + 4 ))
        ;;
      sla71168 )
          # new ABS Pre-Production and Production
          lBackupRetentionPeriod=8
          lBackupRetentionPeriodDb2=8
          if [ "${lInstance}" == "db2bnlr1" ] ; then
            lBackupRetentionPeriod=1
            lBackupRetentionPeriodDb2=1
          fi
        ;;
      sla70193 )
          # ABS Pre-Production and Production
          lBackupRetentionPeriod=10
          lBackupRetentionPeriodDb2=12
          if [ "${lInstance}" == "db2bnlr1" ] ; then
            lBackupRetentionPeriod=1
            lBackupRetentionPeriodDb2=1
          fi
    ...
    #
    # Shadow databases and *99-databases follow a different path
    #
    if [[ ${lDatabase} == DS* ]] ; then
      lBackupRetentionPeriod=1
      lBackupRetentionPeriodDb2=1
    elif [[ ${lDatabase} == *99 ]] ; then
      lBackupRetentionPeriod=1
      lBackupRetentionPeriodDb2=1
    fi
...
#        lBackupCmd="db2 CONNECT TO ${lDatabase} >>#LOG# 2>&1 ;
...
#
typeset lBackupDir="${cBackupDir}/${cHostName}/${lInstance}"
...
chgrp -R ${cDb2CommonSecurityGroup} ${lBackupDir} >/dev/null 2>&1
...
#  [[ "${lSshResult}" =~ "CatalogFile" ]] && lSshUsable="YES"
...
do
  ...
  typeset    lLogOutput="${lLogOutputDir}/${lTimestampToday}_${lDbToHandle}.log"
