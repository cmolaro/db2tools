...
#       -N | --nonrecoverableloadcheck : Include additional check on nonrecoverable load operations
...
# Constants
#
typeset    cCmdSwitchesShort="I:D:U:P:X:C:NqhH"
typeset -l cCmdSwitchesLong="instance:,database:,user:,password:,exclude:,check:,nonrecoverableloadcheck,quiet,help"
  ...
  function getBackupStatus {
    ...
    if [ "${lPerformCalculation}" != "YES" ]; then
      ...
      do
        if [ "${aCheckToday[${lBackupTypeToCheck}]}" == "NO" ] ; then
          ...
          echo "${lReturnedStatus} ${lCheckmkServiceName} age_hours=${lBackupMetrics}; ${lStatusText} - No check configured for today for a ${lBackupTypeToCheck} backup"
        fi
      done
    ...
    if db2 +o connect to "${lDatabase}"; then
      ...
      do
        ...
        set +x
        if [ "${lNonRecoverableLoadCheck}" == "YES" ]; then
          lNonRecoverable=$( db2 -x ${lSqlSpecific} 2>&1 )
	   ...
	   if [ "${lDatabase}" == "DRBNL01" ] ; then
          ...
          if [ $( echo "${lBackupInfo}" | grep '^SQL[0-9][0-9]*[NWC]' | wc -l ) -eq 0 ] ; then
            ...
            echo "${lReturnedStatus} ${lCheckmkServiceName} age_hours=${lBackupMetrics}; ${lStatusText} - ${lBackupAgeHours} hours since last full backup at ${lBackupTimestamp}"
          else
            lBackupMetrics="0;${lBackupMetrics}"
            echo "${lReturnedStatus} ${lCheckmkServiceName} age_hours=${lBackupMetrics}; ${lStatusText} - ${lBackupTypeToCheck} failure fetching the backup timestamp"
          fi
        else
          lReturnedStatus=2
          lStatusText="CRITICAL"
          lBackupMetrics="0;${cBackupThresholdInHoursWarning};${cBackupThresholdInHoursCritical};0"
          echo "${lReturnedStatus} ${lCheckmkServiceName} age_hours=${lBackupMetrics}; ${lStatusText} - ${lBackupTypeToCheck} ${lNonRecoverable}"
        fi
      done
      ...
      echo "${lReturnedStatus} ${lCheckmkServiceName} age_hours=${lBackupMetrics}; ${lStatusText} - ${lBackupTypeToCheck} cannot connect to this database${lUtilityReturnText}"
    fi
...
typeset    lUsername=""
typeset    lPassword=""
typeset -u lNonRecoverableLoadCheck="NO"
  ...
  while [ "$#" ] ; do
    ...
    case ${_lCmdOption} in
      ...
      -C | --check )
        ...
        shift 2
        ;;
      -N | --nonrecoverableloadcheck )
        lNonRecoverableLoadCheck="YES"
        shift 2
        ;;
      -q | --quiet )
        lVerbose="NO"
...
do
  lReturnedText=$( getBackupStatus "${cHostName}" "${lInstance}" "${lDbToHandle}" "${lBackupType}" )
  lReturnedStatus=$?
  [[ ${lReturnedStatus} -lt 10 && "${lReturnedText}" != "" ]] && echo "${lReturnedText}"
done
